...
const FROM = process.env.RESEND_FROM ?? "Emmalva <no-reply@emmalva.local>";
...
export function expiryEmailHtml(name: string, label: string, expiresAt: Date, days: number) {
  ...
  return `
    <div style="font-family: Nunito, Arial, sans-serif; background:#0a101c; color:#eef1f7; padding:32px; border-radius:16px;">
