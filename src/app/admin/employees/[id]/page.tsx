...
export default async function AdminEmployeeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const initial = {
    ...
    profile: {
      ...
      foodCourseExpiry: p?.foodCourseExpiry && p.foodCourseExpiry.getFullYear() > 2098 ? "" : iso(p?.foodCourseExpiry),
      ...
      checkinRadiusM: p?.checkinRadiusM ?? 1000,
      checkoutLat: p?.checkoutLat ?? null,
      checkoutLng: p?.checkoutLng ?? null,
      checkoutRadiusM: p?.checkoutRadiusM ?? 1000,
      ...
      foodCourseFrontBlobUrl: fileUrl(p?.foodCourseFrontBlobUrl) || null,
      foodCourseBackBlobUrl: fileUrl(p?.foodCourseBackBlobUrl) || null,
